...
public class OwnerDao {
    ...
    public String getGoodEmployee() {
        String query = "SELECT a.FullName\n"
                + "FROM Account a\n"
                + "JOIN (\n"
                + "    SELECT IDAccount, COUNT(*) AS BookingCount\n"
                + "    FROM BookingDetails\n"
                + "    GROUP BY IDAccount\n"
                + ") bd ON a.IDAccount = bd.IDAccount\n"
                + "WHERE a.IDRole = 3\n"
                + "ORDER BY bd.BookingCount DESC\n"
                + "OFFSET 0 ROWS\n"
    ...
    public double getRating() {
        String query = "SELECT AVG(CONVERT(FLOAT, Rating)) AS AverageRating\n"
                + "FROM Feedback;";
        try {
            conn = DBContext.getConnection();//mo ket noi
            ps = conn.prepareStatement(query);
